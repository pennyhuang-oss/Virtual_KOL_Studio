#!/usr/bin/env python3
"""日常素材 v1：從 matrix.json 決定性地產生 95 段 prompt。
...
PEOPLE = {
 ...
 "outdoor_background_ok":"A few other people are further back behind her, turned away or in profile and well out of focus, none of their faces readable.",
}
...
DEVICE_TELL = {
 "front_selfie":"Her forearm runs out of the bottom corner of the frame toward the phone, and the angle is a little too close and a little too low, the way a real front-camera shot is.",
...
CAMERA_POS = {
 ...
 "propped_timer":"The phone is resting on a surface roughly three metres from her at about waist height, so the angle looks up at her very slightly and nobody is holding it.",
}

def build(p, slot, light_code):
    L=[]
    L.append(f"A photograph of a young {p['ethnic']} woman, {FRAMING[slot['framing']]}.")
    ...
    if slot["device"]=="found_mirror": L.append(MIRROR)
...
def audit(pid,n,slot,txt,p):
    ...
    if slot["framing"]=="full_length" and "not behind her" not in txt: e.append("§3-E 全身框架句缺失")
    if slot["device"]=="found_mirror" and "no second phone" not in txt: e.append("§3-D① 鏡面排除句缺失")
    if "grain" in txt or "35mm" in txt or "Instagram" in txt: e.append("§3-D② 出現已刪除的尾巴")
    if txt.count(".")<12: e.append("句數過少，恐失去構圖控制")
...
def main():
    ...
    with open('PROMPTS.md','w') as f:
        f.write("# 日常素材 v1 — 95 段 prompt（產生檔，勿手改）\n\n")
