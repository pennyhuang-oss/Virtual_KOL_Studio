...
def build():
    ...
    w("## §6 本輪請你判斷")
    w("")
    w("### 6-1 上一輪（R12）你開的 2 條，處置如下")
    w("")
    w("| ID | 你的判定 | 我做了什麼 |")
    w("|----|---------|-----------|")
    w("| C-48 | 相機模板在戶外仍寫 `room` | **屬實**，實測 9 段戶外／半戶外列中招。改為你建議的 `vertical lines in the scene stay straight and parallel to the frame edges` |")
    w("| C-49 | 仍有三類裁切外身體／手部 cue | **三類全部屬實**。①朝向模板寫 `one hip is nearer the lens` 用在 chest_up 的 a02–a05 與 waist_up 的 c12，共 5 列，已改用肩線與鎖骨當地標；②`a01` 是 face_closeup 卻寫 `with her hands still`，已刪；③`a08` 是 chest_up，我在 visibility 表把「垂在身側」的兩手都標成可見——**這是我填錯**，已改為兩手皆不可見，動作句也改成只描述肩線與上半身 |")
    w("")
    w("### 6-2 你要求的 lint 已補上，並用反向測試驗過")
    w("")
    w("你說「lint 也應新增『可見區域外的 anatomical token』檢查，而不只檢查服裝層與 hands 欄位」。")
    w("已實作：**整段掃描**（不只身材模板），逐 framing 定義裁切外的解剖名詞，")
    w("並豁免服裝規格裡的同形字（`high-waisted`、`wide-leg`、`knee-high` 等）")
    w("以及景別句本身——那句就是在陳述哪些部位落在邊界外，是你 R11 裁決過的有效寫法。")
    w("")
    w("另外補一條：**兩手都在裁切外時，prompt 任何地方都不得出現「hand」**（上臂不算，chest_up 看得到）。")
    w("")
    w("**反向測試 4/4**：把你這輪抓到的四種缺陷逐一打回 prompt，lint 全部擋下。")
    w("")
    w("### 6-3 這一輪請你判斷")
    w("")
    w("**1. C-48／C-49 可否結案？連帶 C-37／C-44。**")
    w("")
    w("**2. 受影響的列**：你指定要重看 `a01`／`a02`／`a03`／`a04`／`a05`／`c12`／`a08`")
    w("   以及 `CAMERA-1` 的展開結果——這幾列與那個模板都在 §5。")
    w("")
    ...
    w("相機句不得假設室內；戶外不得說光來自 room；不得出現第二個 person token；")
    w("每段都要有正面封閉集合收尾。")
    w("")
    w("---")
    w("")
