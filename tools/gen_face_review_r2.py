#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""產生 review/REVIEW_BATCH3_FACES_R2.md —— 請 ChatGPT 複核我對它規劃的整併與修正。

自足檔案。§7 回覆區（自成一行的 REPLIES BELOW 之後）不會被覆蓋。
"""
import json, io, re, subprocess, itertools, hashlib
from collections import Counter

D = json.load(open('pilot/batch3_faces_v2.json', encoding='utf-8'))
P, AX = D['personas'], D['axes']
ALL = list(AX)
B1 = ['kanon-komori', 'yerin-han', 'wendy-yeo', 'angeline-kwee', 'somi-oh', 'jia-seo']
SLOTS = ['FACE_SHAPE_AND_JAW', 'EYES_AND_BROWS', 'NOSE', 'MOUTH']
COMMIT = subprocess.run(['git', 'rev-parse', '--short', 'HEAD'],
                        capture_output=True, text=True).stdout.strip()
dist = lambda a, b: sum(1 for x in ALL if P[a]['axes'][x] != P[b]['axes'][x])
pairs = sorted((dist(a, b), a, b) for a, b in itertools.combinations(P, 2))
inb = sorted((dist(a, b), a, b) for a, b in itertools.combinations(B1, 2))

ex = 'kanon-komori'
old_assign = ("Reference assignment: FACE_SHAPE_AND_JAW from ref_04; EYES_AND_BROWS from ref_05; "
              "NOSE from ref_12; MOUTH from ref_03.")
new_assign = ("Take the face shape and jawline from the first reference image; the eyes and brows "
              "from the second; the nose from the third; the mouth from the fourth.")

o = io.StringIO(); w = o.write
w(f"""# Batch 3 臉部規劃 — R2：請你複核我對你規劃的整併與四項修正
...
for i in range(0, len(ids), 2):
    ...
    for pid in ids[i:i + 2]:
        h = hashlib.sha1(P[pid]['face_en'].encode()).hexdigest()[:8]
        row.append(f"`{pid}` | `{h}`")
...
""".format(old=old_assign, new=new_assign))

for slot in SLOTS:
    c = Counter(P[p]['refs'][slot] for p in P)
    w(f"**{slot}**\n\n| 參考圖 | 供給幾位 | 是哪幾位 |\n|---|---|---|\n")
    for k, v in c.most_common():
        who = [p for p in P if P[p]['refs'][slot] == k]
        w(f"| `{k}` | {v} | {'、'.join(who) if v > 1 else who[0]} |\n")
    w("\n")
...
2. 既然粗分群規則失效，**判定「規格層是否夠分離」的門檻應該改成什麼？**
   ...
   目前的下限是 4 條（{pairs[0][1]} vs {pairs[0][2]}），你覺得夠嗎？
...
你在 (F) 建議先跑 6 位：`{'`、`'.join(B1)}`。

我同意「先驗證方法再展開」這個原則。但我算了一下這 6 位**內部**的兩兩距離：

| | 第一批 6 位內部 | 全體 19 位 |
|---|---|---|
| 最小相異軸數 | **{inb[0][0]} 條**（{inb[0][1]} vs {inb[0][2]}）| **{pairs[0][0]} 條** |
| 中位數 | {inb[len(inb)//2][0]} 條 | {pairs[len(pairs)//2][0]} 條 |

**第一批內部最接近的兩人差 {inb[0][0]} 條，而全體最接近的兩人只差 {pairs[0][0]} 條。**
...
for n, a, b in pairs[:3]:
    where = ('兩位都在第一批' if a in B1 and b in B1
             else '只有一位在第一批' if (a in B1) ^ (b in B1) else '**兩位都不在第一批**')
    w(f"| {n} | {a} vs {b} | {where} |\n")
...
OUT = 'review/REVIEW_BATCH3_FACES_R2.md'
MARK = 'REPLIES BELOW'
body = o.getvalue()
try:
    old = open(OUT, encoding='utf-8').read()
    ls = old.split('\n')
    hits = [i for i, ln in enumerate(ls) if ln.strip() == MARK]
    if hits:
        kept = '\n' + '\n'.join(ls[hits[-1] + 1:])
        if kept.strip():
            body = body.rstrip('\n') + kept
