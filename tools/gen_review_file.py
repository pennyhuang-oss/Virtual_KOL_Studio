...
def build():
    ...
    w("---")
    w("")
    w("## §4 前七輪覆核發生過什麼（你的前任意見與 Claude 犯過的錯）")
    w("")
    w("| 輪次 | 發現的主要問題 |")
    w("|------|---------------|")
    ...
    w("### 8-1 上一輪的結果")
    w("")
    w("你 R7 提的 C-27–C-30 逐條實測全部屬實，已全部修完。最重要的一條是 C-27：")
    w("我原本以為 `hands` 兩個文字槽就夠了，你指出四個漏洞（同義詞繞過、一槽塞多動作、")
    w("沒有可見性關係、鏡像左右不明）——**全部成立**。props 與 hands 已改成結構化，")
    w("而且這個改動順帶讓 C-29 變成機器可稽核的：`zone` × `framing` 對照表一跑，")
    w("裁切外的微物件會直接 FAIL，不必再靠人眼一列一列看。")
    w("")
    w("### 8-2 這一輪請你判斷")
    w("")
    w("1. **結構化設計本身**（§5-5b）：`relation` / `zone` / `object_ref` / `laterality` 四者")
    w("   夠不夠關掉你列的四個漏洞？特別是 `zone` — 我刻意讓它由作者依**該 shot 的實際姿態**指定")
    w("   而不是套站姿公式（蹲著拆箱時地上的紙箱在膝線，不在腳下）。這個自由度會不會反過來變成新的繞過路徑？")
    w("2. **重跑 20/20 九欄語意覆核**：上一輪你判 11 列無異議、9 列 P0，9 列全部改過了。")
    w("   請重新逐列判斷 `scene`／`outfit`／`hair`／`framing`／`view`／`eye_gaze`／`body_pose`／")
    w("   `props`／`hands`／`light` 在物理上是否同時成立。**列出有問題的 shot_id 與理由；其餘列出「無異議」的 id。**")
