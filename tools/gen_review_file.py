...
def build():
    ...
    w("---")
    w("")
    w("## §4 前六輪覆核發生過什麼（你的前任意見與 Claude 犯過的錯）")
    w("")
    w("| 輪次 | 發現的主要問題 |")
    w("|------|---------------|")
    ...
    w("")
    w("> C-23：上一輪 §8 要求逐列判斷 props，本檔卻沒有揭露任何一列的 props——這是本檔的生成漏洞。")
    ...
    w("")
    w("| id | view（誰在拍）| 左手 | 右手 | props | outfit 自帶的包/外套・首飾 |")
    w("|----|--------------|------|------|-------|--------------------------|")
    for x in S:
        o=d['outfits'][x['outfit_id']]; h=x['hands']
        w(f"| `{x['shot_id']}` | {x['view']} | {h['left']} | {h['right']} | {'、'.join(x['props'])} | "
          f"{o['outer_or_bag']}・{o['jewelry']} |")
    w("")
    w("### 5-6 現行分布（程式計算）")
    w("")
    ...
    w("### 8-1 上一輪的兩個爭議點，已結案")
    w("")
    w("- **c04**：接受你的裁決。前鏡頭在螢幕上緣，`looking at phone screen` 與")
    w("  `looking directly into the camera` 對模型是兩個不同指令——我原本用物理去辯，")
    w("  但送進模型的是文字。這是 identity_core 錨點，視線精度優先，")
    w("  因此保留 `eye_gaze=camera`，scene 改寫為「舉起手機直視鏡頭」。")
    w("- **c03**：你同意不改，已結案。")
    w("")
    w("### 8-2 這一輪請你判斷")
    w("")
    w("1. **§7 所有 🔵 的項目**可否結案？其中 C-23 我照你的要求補了 props 表，")
    w("   補上之後立刻看到三類你也還沒看過的錯：8 列 props 重述 outfit 已提供的包或借用別套的招牌包、")
    w("   `c10` 抱著衣物還多一隻手拿零錢、`c07` 把「客人的手」放進訓練集（訓練圖裡不該有第二個人的身體部位）。")
    w("   全部已修，並新增 `hands` 欄位讓它變成機器可稽核的。**請確認這個欄位設計有沒有漏洞。**")
    w("2. **真正的九欄 20/20 語意覆核**（現在 props 與 hands 都在 §5-5b）：")
    w("   請逐列判斷 `scene`／`outfit`／`hair`／`framing`／`view`／`eye_gaze`／`body_pose`／`props`／`hands`／`light`")
    w("   在物理上是否同時成立。**列出有問題的 shot_id 與理由；其餘列出「無異議」的 id。**")
    w("   這一關是 HARD FAIL gate，20/20 之前不會放行生成。")
