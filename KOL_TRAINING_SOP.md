...
## 新角色進庫前的強制關卡（2026-09-07 起）

2026-09-07 完成 19 位 Soul V2 訓練後追加。理由與實測數據見
`PERSONA_CANON.md` 原則六補記與原則七、`review/soul_pilot/SCREEN19_ROOT_CAUSE.md`。

**這 19 位全部沒有 `face_distinct_from`**，而原則六第 2 條本來就強制要求。
結果是 19 位裡 17 位至少和另一位撞臉。這一關就是為了不再發生同一件事。

### 生成 identity_master 之前

1. `face_type` 寫到**骨架幾何**：臉長寬比、眼型與眼距、鼻樑寬窄、唇厚、下顎角度。
   「清冷」「甜美」「鵝蛋臉」這種等級**已實測證明不足**以對抗模型先驗。
2. 填 `face_distinct_from`，指名**距離最近的幾位**並逐一寫明骨架差異。
3. 檢查是否又套用了與其他角色**完全相同**的 `appearance.skin` 硬規格。
   那段字若同時鎖膚色與五官族裔，會壓過每人一行的 `face_type`。

### 生成 identity_master 之後、送訓之前

4. 跑配對距離篩檢（**0 credits**，工具見 `review/soul_pilot/_collision_v1/`）：
   mediapipe landmarks → 置中與 RMS 尺度正規化 → SVD Procrustes 對齊 → 平均 RMS 距離。
5. 任何 ≤**0.0220** 的配對，把兩位的 master **遮掉髮型髮色**並排目視複核。
   校準點：0.0157 是實測**盲測失敗**的距離。
6. 距離過不了就**改骨架重生 identity**，不要靠改髮型或妝容——原則六第 3 條。

**這一關 0 credits。跳過它的代價是 25 credits 一位的重訓，加上整批的信任。**

### 送訓時的操作要點（本批實戰得到）

- 每位送訓前**重新上傳**取新 `media_id`，並逐張 SHA-256 比對伺服器端 bytes。
  本批 96/96 相符。
- 對帳只看**扣款種類與筆數**：`Soul ID -25` 才是受理；
  重複 `Higgsfield Soul V2 -0.12` 是扣款但失敗。**絕不看 `balance`**——
  本批全程有另一位使用者並行使用同一帳號，餘額對帳一次都對不起來。
- train 呼叫**間隔拉開，或一批最多送 2 個**。本批兩次 `Something went wrong`
  都發生在連續三次呼叫的第三次（n=2，未證實，但緩解措施很便宜）。
- 呼叫失敗時**先查再送**：確認無 `Soul ID -25`、無角色記錄，**等滿 15 分鐘**，
  才發唯一一次重送。本批兩次失敗都因此只被扣一次款。
- **5 張訓練集足夠**（本批實測）。此帳號先前每個成功的 soul 都用 12–13 張。
