...
### 3-F 第三次補正：單人排除句在「人多的戶外場景」會失效（2026-09-07）

§3-F 原本把「否定人」列為唯一有效的例外：
`She is the only person in the photograph; no other people are visible anywhere in the frame.`
到目前為止在室內與可控場景一直有效。

**新證據推翻它的普適性。**

cheryl-soh 驗證圖 V5：商店街、陰天、走路。同一句排除句原封不動放在句尾，
成品背景仍出現 4–5 個路人。

補正後的規則：

- **室內、可控、本來就不該有人的場景** → 排除句有效，繼續用。
- **本來就會有人的戶外場景**（商店街、車站、市場、公園步道）→
  排除句**擋不住**。模型會照場景常識填人。

**這不必然是缺陷。** V5 的路人全部嚴重失焦，原生解析度 4× 放大後沒有任何一張臉
可辨識，看起來就是真實街拍該有的樣子；硬要清場反而更假。

**真正要守的線是 §QA-1 的線，不是「有沒有人」：**
背景人物只要**臉可辨識**就不能收（angeline #3 是清晰約 180×240px 的螢幕人臉，
已丟棄重生）；失焦到認不出臉的人形不算違規。

所以：戶外街景**不要**指望排除句清場，**要**在原生解析度檢查有沒有可辨識的臉。
若這一張確實需要無人，改寫場景本身（清晨、後巷、非營業時段），不要靠加否定句。

---

## 出圖必檢：C-1 髮型、C-2 不同框（2026-09-07 起強制）

規則出處與完整理由見 `PERSONA_CANON.md` 原則七。這裡只放出圖時要照著做的動作。

### C-1 每一則 prompt 都要檢查

- [ ] **有寫該位自己的髮色與髮型？** 沒寫會隨機，隨機就可能與她的碰撞對象撞。
      這 19 位的髮型是**承重的**——Soul V2 不繼承髮型，所以它是每次生成
      唯一能把她和長得像的人分開的東西。
- [ ] 若這次要換髮型（原則三允許換，用來製造時間感）：
      先查 `review/soul_pilot/_screen19_v1/screen19_pairs.json`，
      **新髮型不得與距離 ≤0.0220 的任一對象相同或相近。**

### C-2 排程時要檢查

- [ ] 這一則內容裡**只有一位**碰撞名單上的人設？
      同圖、同片、同一組輪播都算同框。並排會直接暴露相似度。

### 不要誤讀這兩條

C-1／C-2 **不是**撞臉的解決方案，是已接受風險下的損害控制。
`PERSONA_CANON.md` 原則六第 3 條仍然成立且已被實測證實：
**撞臉只能改骨架，改髮型改妝容都沒用。** 骨架問題仍然存在、仍未修。

### 最需要注意的人設

碰撞配對最多的前幾位：**ruoruo-tang（11 組）、sydney-leong（10）、
nanami-fujiwara（9）、miu-shiraishi（7）、jia-seo／cheryl-soh／zhiyi-shen（各 6）**。
