...
**短髮角色（Luna 的鮑伯）要多加一句正面描述兩側等長**：
`chin-length black bob, both sides exactly the same length`。
不能寫 `not asymmetric`——否定句無效（見第 23 點）。

> ⚠️ **這個修法本身還沒被驗證過。**下一次生 Luna 的第一張，要專門檢查這一項。
...
**第一段——瑕疵掃描（只要中一項就直接淘汰，不用往下評）**

| 檢查 | 看什麼 |
|---|---|
| **髮長對稱** | 兩側長度一致嗎？有沒有多出一撮特別長的？ |
| **手指** | 每隻手 5 指？有沒有多指、少指、融合、長度異常？ |
| **四肢** | 手臂腿的數量與關節方向對嗎？ |
| **道具連貫** | 拿的東西跟 prompt 一致嗎？有沒有半透明、穿模、憑空接續？ |
| **臉部對稱** | 兩眼大小、耳朵高度、眼鏡腿有沒有斷 |
| **文字** | 招牌與包裝上的字有沒有變成亂碼（要發布的話尤其重要） |
| **背景人物** | 有沒有黏在一起、少一條腿、臉糊成一團 |
| **衣物邏輯** | 領口、袖口、腰帶有沒有接不起來；鞋襪有沒有穿模 |
