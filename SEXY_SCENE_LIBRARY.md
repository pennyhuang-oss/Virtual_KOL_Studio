...
#### 24-F. 要排除某物，就描述一個讓它沒機會存在的**可見邊界**（2026-09-01）

**D-05 已驗證「否定句無效」，但一直沒回答：那要排除東西時該怎麼寫？**

### 起因：鏡 3 的手

prompt 寫 `both hands below the frame`（雙手在畫面下方之外）。**2/2 都生出雙手捧碗。**

> 「`both hands below the frame` **只描述不可見結果**，
> 已證明會被更強的『端碗』場景語意覆蓋。」

**而且替代方案也不行**：改寫成「雙手放在腿上」**仍然把模型的注意力叫到兩隻手**，
而且位置不可驗收，可能反向生成膝上手或把景別拉遠。

### 正解：描述裁切邊界本身

```
the lower frame edge crosses both upper arms above the elbows
（畫面下緣切過雙上臂、在手肘以上）
```
...
| 想排除 | ❌ 不要寫 | ✅ 改寫成 |
|---|---|---|
| 手 | 手在畫面外／手放腿上 | **畫面下緣切過上臂、在手肘以上** |
| 任何肢體或物件 | 它不在畫面裡 | **一條可見的裁切線，位置在它出現之前** |

**關鍵差別**：前者要模型先生成它、再把它藏起來；
後者讓它**在解剖與構圖上沒有進入畫面的條件**。

⚠️ **n=0，尚未實測。**這是 R22 的設計，下一批 2 張就是它的第一次驗證。
...
##### 最重要的一條：先寫相機，再視需要補身體朝向（這是順序，不是禁令）

每段的寫法順序是 **① 方位 → ② 高度與距離 → ③ 只在硬驗收需要時才補身體朝向**。

LG-07 同一件、同一個 soul 的對照如下：
...
> **⚠️ 2026-08-29 外部覆核（R14）修正：這組對照不能當成單變因證據。**
> 原文：「LG-07 這組對照**同時改了句型、語意強度與任務難度**，只有 2/2 對 2/2，
> **不能分離出唯一因果**⋯⋯遇到回頭、手物接觸或臉必須清楚時，兩者仍可能都需要。」
>
> **可以保留的結論**：先給相機一個明確的方位，比只描述身體朝向可靠。
> **不可以保留的結論**：「身體朝向不要寫」。回頭、手物接觸、臉必須清楚這三種硬驗收，
> 方位與身體朝向**兩者都要寫**。
...
**方位**：有一組同件同 soul 的對照（LG-07 v1/v2），但依 R14，該對照同時動了句型、
語意強度與任務難度，只能支持「要寫方位」，不能支持「不要寫身體朝向」。
列為**方向可信、因果未分離**。
...
**推測**：「束起」是一個狀態，模型沒有可依附的實體；「用鯊魚夾夾起」有一個要畫出來的東西。
⚠️ n=2，兩件不同角色不同場景，**不是嚴謹的單變因對照**，列為待驗證的傾向。
