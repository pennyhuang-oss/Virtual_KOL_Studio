...
## 2026-07-25 臉部/風格選角候選批次（Discovery Batch，已生成，等待使用者挑選，尚未建立 Reference Element，尚未送入 Soul 訓練）

**狀態：⚠️ 等待使用者從 4 張候選圖挑出最喜歡的一張臉／風格 —— 尚未建立 Reference Element、尚未執行 Soul 訓練、`profile.json` 沒有 soul_id，也不會在這輪自動送訓。**
...
| 檔名 | 角度／景別 | Job ID |
|------|-----------|--------|
| candidate_01.png | 正面臉部特寫（headshot） | `29e39fae-8856-4b6d-8846-650dcc3a6bbb` |
| candidate_02.png | 正面半身（half-body） | `68b88057-97f3-4cad-895e-e68b89bfad8c` |
| candidate_03.png | 四分之三側半身（3/4 half-body） | `9624ce6e-f4c6-4419-888e-3a066671e006` |
| candidate_04.png | 正面全身（full-body，實際出圖為正面半身偏下的裁切，未完全到腳，鏡頭指令未 100% 依照角度描述執行） | `9a6aeee7-6239-45fd-a875-fd461b6bf638` |
...
**產出檔案**（`kols/rainie-hsu/images/face_reference/`，共 4 張，已用 Read 工具逐張目視檢查）：candidate_01–04 皆呈現「美豔動人、驚艷大氣」的臉型方向（非銳利冷硬），大而有神的雙眼搭配精緻貓眼眼線、正紅唇色，四張圖之間臉部特徵、髮型、服裝、配件與燈光風格判讀一致；candidate_04 的實際出圖鏡頭裁切比預期更接近半身而非到腳的全身，鏡頭指令未完全被模型遵守，但不影響本批次「臉部/風格選角」的目的。

**⚠️ 下一步（不可跳過）**：依照 README.md「新增 KOL 流程」與 `KOL_TRAINING_SOP.md` 的強制規則，**必須停下來，等使用者實際看過這 4 張候選圖並明確指出最喜歡的一張（或說明皆不滿意需要重新調整）後，才可以進入下一階段**——下一階段流程為：(1) 將使用者核准的單一一張圖上傳並建立 Reference Element，(2) 以該 Element 錨定身分，重新生成一組完整的一致性訓練圖／素材（比照 Vicky Lin 第四輪的做法），(3) 訓練圖經確認後才呼叫 `show_characters(action='train')` 建立 soul_id。本輪**沒有**建立 Reference Element，**沒有**呼叫 `show_characters(action='train')`，`profile.json` 的 `ai_generation`／soul_id 維持原狀未變更，訓練狀態明確標記為 **PENDING**。
