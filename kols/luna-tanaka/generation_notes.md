...
**背景**：兩位人設要開社群帳號（見 `clients/sushisolar-rujiao/POSTING_PLAN.md`），
發布前把 repo 內既有素材逐件打開檢查。

### 🔴 新發現的瑕疵：`soul_test_v1` 咖啡廳兩張有 AI 生成的假 Instagram 介面

- `images/soul_test_v1/03_cafe_window_01.png`
- `images/soul_test_v1/04_cafe_window_02.png`

兩張圖的畫面上都被模型畫進了**假的 Instagram Story 介面**：
左上角有頭像圈＋亂碼帳號名（分別是「Plaremgilem1 1.gj」與「algpaden」），
右上角有 ✕ 關閉鈕。

**判定：不可發布。**發出去會像一張壞掉的截圖。
（這兩張仍是當初重新訓練 v2 `soul_id` 的七張核准照片之一，**訓練用途不受影響**——
模型學到的是臉，不是那層 UI；但作為對外發布的素材不能用。）

> **教訓**：訓練用核准 ≠ 可發布。當初核准這批是為了確認身分一致性，
> 沒有以「發出去給人看」的標準檢查過畫面雜訊。往後素材入庫時，
> 建議把「訓練可用」與「可發布」分成兩個獨立的核准欄位。

### 場景重複度

和室（`dance_clone_r1`/`r16`/`r17` 的起始畫面與影片）合計出現 **5 次**，
是這批素材最集中的場景。發布序列已刻意把它們拉開（L-04 → L-07 → L-09 → L-14），
`r16` 影片留備用不發。

### `dance_v1`（霓虹居酒屋・黑皮洋裝）不建議發布

1. 畫面左右兩側有模糊填充條，是比例轉換的殘留；
2. 黑皮洋裝＋霓虹夜店的調性不符合 Luna 的人設世界（低飽和、奶白、安靜），
   較接近 Rainie Hsu 的派對女王路線。

### ✅ 舞蹈影片配樂授權問題撤銷

先前 Step 7 記載「配樂未取得商用授權，發佈前必須替換」——**此顧慮不成立**
（Penny 2026-08-25 確認）：這些是社群平台上做 Challenge 的熱門音訊，
發布後平台會自動辨識並帶上版權資訊；驅動片本身也是別人用同一首歌發的 Challenge 影片，
跟著做反而是幫音樂宣傳。**舞蹈影片可直接發布，不需換配樂或重新對拍。**

### 2026-08-25 Penny 審核結果（發布用途）

| 素材 | 判定 | 理由（Penny 原話） |
|---|---|---|
| `soul_test_v1/01,02_tokyo_street`（LA-01／02） | **採用** | 併成一則 carousel |
| `soul_test_v1/03,04_cafe_window`（LA-03／04） | 不可用 | 生成瑕疵：假 Instagram 介面（見上一節） |
| `soul_test_v1/05,06_park_golden`（LA-05／06） | **採用** | 併成一則 carousel |
| `dance_clone_r1/start_frame`（LA-07） | 未採用 | 不知道意圖是什麼，背景很不好看 |
| `dance_clone_r1/start_frame_alt`（LA-08） | 不可用 | 雜誌殘留條 |
| `dance_clone_r16/start_frame`（LA-09） | 未採用 | 背景不好看 |
| `dance_clone_r17/start_frame`（LA-10） | 未採用 | — |
| `videos/daily_v1`（LA-11） | **備用** | 意圖有點不太明確，留著但不排入 |
| `videos/dance_clone_r1`（LA-12） | 未採用 | 背景不好看 |
| `videos/dance_clone_r16`（LA-13） | 未採用 | — |
| `videos/dance_clone_r17`（LA-14） | 未採用 | — |
| `videos/dance_v1`（LA-15） | 不可用 | 邊緣填充條＋調性不符 |

> **和室場景六件（LA-07／09／10／12／13／14）全數未採用，理由集中在「背景不好看」。**
> 這是這批素材最大的失分點——當初為了訓練一致性在同一個房間連拍，
> 對建模是對的，但那個空間本身撐不起社群畫面。

**通過審核 4 件 → 併成 2 則 carousel。**

---

## 2026-08-25 生成準則（兩位共用，由審核結果反推）

往後每一批生成前必須檢查這三條：

1. **背景決定成敗。** 和室六件全被否，理由都是背景。
   **場景是第一優先**，不要在同一個房間連拍一整批。
2. **畫面要有正在發生的事。** Yuna 的 YA-01／YA-07 被評「沒什麼含金量」——
   都是純站姿或純自拍，沒有情境。與 `DAILY_VIDEO_SOP.md` 競品數據的
   「情境 > 美貌」結論一致。
3. **一個場景一件，不要同 prompt 連生多張。**
   這兩批之所以只剩約五分之一能用，就是因為當初刻意重複。

> **根本問題：訓練用核准 ≠ 可發布核准。**
> 建議素材入庫時分成兩個獨立欄位，訓練批次不要直接當內容庫用。

---

## 2026-08-25 視覺方向修正（Penny 定調）

**先前把她的美學做成「京都侘寂／老木頭／舊陶器／褪色底片」是方向錯誤。**
使用者反饋：那些背景「很醜」「非常不精緻」，不是她想要的樣子。

**改成東京可愛系**——可愛、精緻、明亮、乾淨。想像的是「會去東京迪士尼打卡、
穿得很可愛出現在澀谷街頭」的那種日系女生，不是京都文青。

生成時最關鍵的三個改動：

1. **場景**：甜點店、可愛系街區、遊樂園、花季、白色系房間、和服祭典
   ← 取代二手書店、老陶器店、苔蘚庭院、榻榻米暗房
2. **色調**：乾淨明亮的奶白＋淺粉、通透。**移除 `haze`**——那是畫面顯髒的主因
3. **`porcelain skin` 仍然禁用**（這條不變，是反 AI 感規則）

`character.md` 已加上「視覺方向修正」章節並改寫視覺美學、主場場景、後製風格、
光線配方、台北場景對照表等段落。

> **順帶撤回**：我先前在規劃書寫過「Luna 結構性不可能有 A 級場景」——
> 那句話本身就是把美學設死的錯誤示範。新方向下遊樂園、祭典、花季都是很自然的 A 級。

### 通用教訓：人設檔的美學描述會直接決定生成品質

這次的問題不是 prompt 寫得不好，是**人設檔本身把方向定錯了**，
於是每一次規劃都被拉回同一個坑。→ 生成結果連續被否時，
先回頭檢查人設檔的美學段落，不要只在 prompt 層打轉。

### 2026-08-25 穿搭校正：把「C 級場景」誤讀成「C 級造型」

批次 1 的整批穿搭被使用者評「**太樸素、看起來很像路人**」。回頭對照原設定，
發現規劃時漏掉的是**人設最核心的賣點**：

- `DAILY_VIDEO_SOP.md` 的「受眾設定（必讀）」第一條：**受眾都是男性，
  服裝選擇要展示身材曲線、乳溝、腰線**
- Luna 的 `character.md` 用粗體寫「**身材是童顏巨乳——臉部非常年輕，
  但身材曲線好、胸部飽滿、腰細**」，而規劃卻整批給她 oversized 針織（袖子過長）、
  長袖睡衣、及膝長裙——**把這個特徵完全蓋掉**
- Yuna 168cm、腿長 82cm 佔身高 48.8%，卻被安排穿卡其工裝寬褲

**根本原因：把「C 級（完全不美的日常）**場景**」誤讀成「不美的**造型**」。**
`WARDROBE_SYSTEM.md` 的 C 級講的是**地點層級**，不是造型層級——
競品 Sherry 敢發 Costco，但**她在 Costco 裡穿的不是路人裝**。
**地點可以土，人不能土**，那個反差正是「她是個剛好很好看的真人」的效果來源。

**修正方向**：高腰、短版、貼身針織、迷你裙、細肩帶、方領、收腰。
oversized 單品只當外層披著，裡面一定合身。兩人的 `character.md` 服裝公式已加補記。

> **給往後規劃的檢查項**：排完一批穿搭後問一句——
> **「這批有讓她的身材特徵出來嗎？」**
> Yuna 看腿長與腰線，Luna 看童顏巨乳的反差。答案是否定就退回重排。
