...
- **最終處理**：從第四輪三連拼貼圖裡裁出第一格（品質最好、五官/手勢最清楚），用 `PIL` 以臉部為中心裁成
  9:16 直式比例（764×1360，2x 放大），存回 `start_frame.png`。**已知取捨**：裁法比其他支的三分身/胸上
  構圖更緊（幾乎是臉部特寫，洋裝下半部大部分不在框內），因為單一格的原始高度不夠再裁出更寬的畫面；
  曾嘗試跨格拼接以保留更多身體範圍，但兩格之間手勢/髮絲位置有明顯不連續的接縫，畫面感更差，故放棄改回
  單格裁法。身分辨識度、服裝色調與領口設計仍清楚可見，Motion Control 主要動作驅動來源也是驅動片本身
  的胸上取景，理論上仍可運作，但次級動態載體（裙擺/腰帶）在起始畫面裡幾乎看不到，待 Step 5 生成後
  重點檢查
