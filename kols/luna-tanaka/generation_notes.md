...
- **用 `count:4` 批次抽樣測試模型隨機性**：R16、R17 各自用同一組 prompt 再生成 4 張變體（總計 12 次
  ...
  總會抽到一張正常的」這條路
- **✅ 最終解法：換用 `nano_banana_pro` + 參考圖，不用 soul_id**——問題根源是 `soul_id` 本身的角色嵌入，
  換一個完全不經過這個嵌入的生成路徑就能繞開：把 `kols/luna-tanaka/images/dance_clone_r1/start_frame.png`
  （既有已核准圖）上傳當 `medias` 參考圖（`role: image`），改用 `nano_banana_pro` 模型（而非
  `soul_2` + `soul_id`）生成，prompt 描述「用參考圖的臉部身分，生成新的完整照片」＋目標服裝/場景。
  R16、R17 各一次呼叫即成功，皆為乾淨的單張三分身全身照，洋裝/連帽裙版型完整可見，身分辨識度良好
  （髮型、臉型與既有素材一致）。**零額外訓練成本，不需要走 37–38 credits 的重新訓練 Soul 路線**
- **記錄供未來參考**：如果之後 Luna 其他批次又遇到同樣的拼貼慣性，此為已驗證的免費解法——改用
  `nano_banana_pro`（或其他支援 `medias: [{role: image}]` 參考圖輸入的模型）+ 既有核准圖當參考，
  而非堅持用 `soul_2` + `soul_id`
