...
## `daily_expression_v5` — 表情系統首次驗證批次（6 張＋3 張重生，2026-08-14 已生成）

**狀態：3 張通過、3 張表情失敗（模型能力邊界，見下方結果章節）。**
...
### 生成結果（2026-08-14，共 9 次生成）

使用者在核准生成時追加一項要求：「**動作可能都要帶有一些勾引、賣弄性感的那種感覺，就是要這種的**」。
檢視表情庫才發現 E-1～E-10 裡**只有 E-1 是明確勾引的**，其餘九個都是「生活感的臉」——表情庫缺了
一整個類別。因此先補上勾引組 **E-11 咬下唇／E-12 微張嘴呼氣／E-13 手指抵下唇**（見
`EXPRESSION_SYSTEM.md`），再依此重排本批表情。

| # | 場景 | 指定表情 | 結果 | Job ID |
|---|---|---|---|---|
| 01 | 淋浴間玻璃內、濕透白背心、手貼玻璃 | E-11 咬下唇 | ❌ **兩次都失敗**（第一次閉嘴微笑、第二次開口大笑） | `19ea2550…` / `44343a1b…` |
| 02 | 廚房流理台上坐著、襯衫扣兩顆、冰箱冷光 | E-5 抿嘴憋笑 | ❌ 第一次閉嘴微笑；第二次加「手抬到嘴邊」勉強有一點但不到位，**且服裝生成超出既定尺度範圍，不採用** | `95163336…` / `c209cce5…` |
| 03 | 沙發扶手後仰伸展、黑蕾絲 | E-12 微張嘴呼氣 | ❌ 第一次閉嘴微笑；第二次仰頭與喉頸展示做到了，但嘴變成**大笑**，與 06 撞表情 | `29588621…` / `927b2cb7…` |
| 04 | 背貼落地窗、雙手舉高貼玻璃、酒紅蕾絲＋大腿襪 | E-1 半瞇微笑（招牌） | ✅ **通過**。展示機制完全成立（舉臂拉開肋廓與腰線），夜感佳 | `1d1ddeba…` |
| 05 | 地毯趴姿翹腿、低雙辮、擦腳指甲油 | E-13 手指抵下唇 | ✅ **通過，本批最好的一張**。表情與前 20 張完全不同，頭低眼上、手指抵唇、相機仍維持平視 | `f7f556ca…` |
| 06 | 浴室洗手台上坐著、絲質睡裙肩帶滑落 | E-4 開口大笑 | ✅ **通過**。這是 Iris 累計 26 張裡**第一張真正笑開、露出牙齒**的照片 | `1ea57c93…` |

### 🔴 本批最重要的結論：模型生不出「純肌肉細節」的細微表情

三張失敗的共同點是**表情屬於純肌肉的細微變化**（咬唇、憋笑、微張嘴呼氣），失敗模式高度一致：
**模型直接忽略，倒退回它的預設臉——閉嘴微笑或開口大笑。而那個閉嘴微笑正是本系統要解決的「一號表情」本人。**

第二次重生已排除 prompt 寫法的可能：即使 (a) 把表情段落搬到 prompt 最前面、(b) 寫成「這張照片唯一
必須呈現的就是 X」、(c) 加上「如果只是閉嘴微笑就算失敗」的否定句、(d) 拉近景別到半身，三個細微表情
**仍然全部失敗**。判定為**模型能力邊界，不是描述精度問題，不應再重試**。

成功的三張則印證了另一面：
- **E-4 大笑**＝臉部大幅形變 → 生得出來
- **E-13 手指抵下唇**＝有**實體物件（手指）**參與表情 → 生得出來，且是本批表情最新鮮的一張
- **E-1 半瞇微笑**＝模型預設臉 → 本來就生得出來

**修正後的規則已寫入 `EXPRESSION_SYSTEM.md` 第四之二節**：表情必須落在「①大動作」或「②有實體錨點」
兩類之一，純肌肉細節（咬唇／憋笑／呼氣／似笑非笑／嘴角微揚）不可單獨使用。
**這條同時影響 Sophia（只有眼睛在笑）與 Luna（嘴角極小幅度上揚）的表情人格設定**——那兩種克制感
目前的模型做不到，要改用實體錨點達成。

另外補一條：`.claude/agents/emotion-director.md` 早就寫過「微表情在全身遠景裡看不見，要求更近的景別」，
但這條之前只用在影片。本批失敗的三張初版全部是中景到全身。**表情批次的臉必須在畫面裡夠大**（胸上到
腰上），已補進 `EXPRESSION_SYSTEM.md`。

### 產出檔案

- `kols/iris-chen/images/daily_expression_v5/04_window_glass_E1_smirk.png`（通過）
- `kols/iris-chen/images/daily_expression_v5/05_rug_prone_E13_finger_lip.png`（通過）
- `kols/iris-chen/images/daily_expression_v5/06_bathroom_counter_E4_laugh.png`（通過）
- `expression_failed/` 內 6 張留檔（01a/01b、02a/02b、03a/03b），**表情未達成，不可作為表情系統的成果使用**；
  其中 02b 另有服裝超出既定尺度範圍的問題，標記為不採用

**狀態：3 張通過待使用者核准；3 個場景待改用「有實體錨點」的表情重做（方案待使用者選定）。**

---

## `selfie_expression_v6` — 自拍常見表情組（6 張，1 個 setup 只變表情，2026-08-14）

**使用者需求**：「安排一些眨眼、吐舌頭、裝可愛那種的，或是性感迷離的眼神不要笑嘴唇微張之類的，
還有嘟嘟嘴，其實就是一些美女常常自拍會有的表情」。

**格式**：因為這批是**純表情測試**，臉必須夠大（見 v5 結論），所以採用 `SEXY_SCENE_LIBRARY.md`
第 14 點指定的 **1 個 setup × 6 種表情** carousel 格式——固定服裝／場景／光線／髮型，**只變表情與手勢**。
**這是工作室第一次實際做出這個格式的素材**（第 14 點列為最高操作優先級，但先前從未執行）。

**固定 setup**：夜晚臥室床上跪坐、黑色蕾絲三角內衣（左肩帶滑落）＋細金鎖骨鍊、長直髮中分放下、
床頭暖燈為唯一光源、房間其餘壓黑、前鏡頭自拍微俯角、景別腰上到胸上。

### 結果

| 表情 | 錨點類型 | 結果 | Job ID |
|---|---|---|---|
| **E-14 眨眼** | 眼睛（閉一隻） | ✅ **成功** | `66fe8f66…` |
| **E-15 吐舌頭** | 新物件（舌頭） | ✅ **成功** | `e3a49bcf…` |
| **E-17 雙手食指戳臉頰（裝可愛）** | 手進入畫面壓變形 | ✅ **成功**，雙手放大檢視五指正確 | `f041fa60…` |
| **E-19 咬髮尾** | 物件（髮絲）進嘴 | 🟡 **動作成功**（髮絲確實咬在齒間、手拉著），但嘴形底色仍是笑 | `78c5a36a…` |
| **E-16 嘟嘴** | 只改嘴形 | ❌ **失敗**，變成閉嘴微笑 | `15dc30bf…` |
| **E-18 迷離不笑、嘴唇微張** | 只改嘴形 | ❌ **失敗**，變成露牙笑 | `2a2070b9…` |

### 🔴 結論：真正的分界是「有沒有動到嘴巴以外的東西」

v5 的結論是「大動作 vs 純肌肉細節」，本批把線切得更準：

> **這個 soul 的嘴巴幾乎鎖死在「微笑／笑」。凡是只靠嘴形變化的表情一律失敗；
> 能成功的全部是靠眼睛、手、或外部物件達成的。**
...
**「不笑」跨兩輪共試三次（v5 的 E-12 兩次、本批 E-18 一次）全部失敗**，每次都用了最強寫法
（表情放 prompt 最前、寫「唯一必須呈現的就是 X」、加「如果在笑就算失敗」的否定句、景別拉到胸上），
三次都生出笑臉。**判定為 `soul_2` + 此 `soul_id` 的硬限制，不再重試。**

**替代路徑（尚未驗證，待使用者決定要不要試）**：
1. 換模型（不用 `soul_2`，改用其他 image 模型＋Reference Element 錨定身分）看嘴部是否解鎖
2. 從影片抽幀——影片動作過程中一定會經過「不笑」的幀
3. 用物件強制改變嘴形（含吸管、棒棒糖、叼眼鏡腳），嘟嘴也適用同一招

### 產出檔案

- `kols/iris-chen/images/selfie_expression_v6/E14_wink.png`（通過）
- `kols/iris-chen/images/selfie_expression_v6/E15_tongue_out.png`（通過）
- `kols/iris-chen/images/selfie_expression_v6/E17_cheek_squeeze_cute.png`（通過）
- `kols/iris-chen/images/selfie_expression_v6/E19_bite_hair.png`（動作可用，表情底色是笑）
- `expression_failed/` 內 2 張留檔（E16 嘟嘴、E18 迷離），表情未達成

**狀態：3 張通過、1 張部分可用，待使用者核准。** 規則已寫入 `EXPRESSION_SYSTEM.md` 第四之三節。
