...
## 2026-08-12 舞蹈克隆 U 批次（使用者自備 3 支驅動片）— Step 1–4 完成，起始畫面待核准

**背景**：使用者一次提供三支驅動片（兩支直接上傳 mp4、一支 IG Reel 連結），要求用 Iris 的人設重做
跳舞影片，並明確指示「先生成三支影片的各起始圖，起始圖 OK 之後再送去生成影片」——即
`DANCE_CLONE_SOP.md` Step 4 的人工核准關卡。批次代號 `u1`/`u2`/`u3`（u = 使用者自備，與 GitHub
Issue #3 的 R 編號批次分開，避免編號衝突）。

### Step 1–2：下載、裁剪、編碼正規化

三支驅動片**原始編碼全部是 VP9**，依 SOP 2026-08-07 規則一律轉 H.264（`libx264` + `yuv420p`），
避免 Mia Huang R1 那種 `motion_control` 無錯誤訊息直接 `failed` 的問題。

| 代號 | 來源 | 原始規格 | 處理 | 內容 |
|------|------|---------|------|------|
| u1 | 使用者上傳 `...20250819...mp4` | 884×1572、VP9、7.22s、59.94fps | 裁掉右側編輯 App 圖示欄與頂部音樂膠囊（`crop=769:1367:0:126`）→ 768×1366 H.264 | 華麗飯店/宴會廳式室內（金線壁板、花紋地毯），短鮑伯頭+瀏海，黑色深 V 繫頸連身泳裝，高能量手勢舞 |
| u2 | 使用者上傳 `...20260308...mp4` | 1080×1920、VP9、14.10s | 裁掉右側圖示欄（`crop=950:1689:0:0`）→ 950×1688 H.264 | 越南夜間街邊（機車排、越南文招牌），短鮑伯頭+瀏海，白色長袖 crop top（領口哥德字樣）+黑色寬版抽繩褲 |
| u3 | `https://www.instagram.com/reel/Da74LRjxOjc/`（`yt-dlp` 下載成功） | 576×1024、VP9、16.47s、30fps | **畫面本身乾淨無 UI**，直接轉 H.264，未裁切 | 明亮居家公寓（天花板嵌入式冷氣、白牆走道），長波浪捲髮+瀏海，奶油黃無袖高領上衣+淺藍牛仔褲，手勢舞 |

三支的原始音軌都已抽出（`A/B/C_audio.m4a`）備 Step 6 用。驅動片原始檔與音軌只存本機工作資料夾，
依 SOP Step 7 規定不進 repo。

### Step 4：起始畫面（`soul_2` + `soul_id: 5fe3b6ba-1277-4822-9141-fb06eb3b93a0`，各 `count=1`）

三張皆**一次生成成功**，1152×2048、無拼貼bug、五官與身分三張互相一致。

| 代號 | Job ID | 場景 | 服裝 | 髮型 |
|------|--------|------|------|------|
| u1 | `899b2f47-0dff-4726-b507-c8e15b6e7cd6` | 台北飯店套房（未整理的床、半開行李箱、床頭燈、落地窗市景） | 黑色繫頸無袖針織上衣 + 黑色高腰緞面短褲 + 小金圈耳環 | 長黑直髮撥到肩後 |
| u2 | `a3de3d18-08d3-41a6-8452-02ecab7d94a1` | 台北夜間巷弄（機車排、鐵捲門、便利商店亮面、背景失焦路人） | 白色貼身長袖 mock-neck crop top（刻意不寫任何字樣，避開 R15 文字亂碼）+ 黑色寬版工裝褲 + 斜背小包 | 低馬尾 |
| u3 | `9579216e-92e0-4673-b974-d9c6f1de5aec` | 台北公寓客廳（米色沙發、皺毯子、矮几上水瓶與趴著的手機、地上充電線） | 奶油黃羅紋無袖上衣 + 淺藍直筒牛仔褲 + 細金項鍊 | 長髮微捲中分放前 |

**三張場景/穿搭/髮型刻意全部錯開**，符合 `SEXY_SCENE_LIBRARY.md` 第 13 點造型差異化與
`DANCE_CLONE_SOP.md`「每批次之間刻意輪替場景」要求；光線段落三張都寫滿五段式物理規格
（主光／具名反射面／兩個色溫／犧牲區／遮擋框架）。

### 送 Step 5 之前必須讓使用者裁決的三個風險

1. **景別比預期寬**：prompt 都寫了 `THREE QUARTER SHOT, mid-thigh up`，但三張實際渲染到**膝蓋上下**。
   依 `DANCE_VIDEO_SOP.md` 已知問題表，起始畫面越接近全身，影片越可能在膝蓋處截斷腿部。
2. **髮型輪廓不匹配（R12 身分覆蓋風險）**：u1、u2 的驅動片本人都是**短鮑伯頭+瀏海**，Iris 是長髮，
   屬 R12（Coco Wu）同類風險。依 R13–R18 批次驗證，此風險是機率性的（6 支中 5 支標記風險、實際 0 支
   發生），可選擇直接跑並在 Step 8 加強比對。u3 驅動片本人是長捲髮，起始畫面已刻意配成長微捲髮，匹配良好。
3. **露腹服裝的像素審核風險**：`DANCE_VIDEO_SOP.md` 已知問題表記載 start_image 的 crop top 露腹曾觸發
   像素層級內容審核導致 `status: failed`。三張成品都有不同程度的腰腹外露（u2 最明顯）。Motion Control
   失敗會全額退款（R6 案例已驗證），成本風險低，但會多花一輪時間。

**目前狀態：三張起始畫面已生成並存入 repo，等待使用者核准後才進 Step 5 Motion Control。**
...
### Step 3：驅動片動作/情緒分析（本輪由主 session 直接看幀完成，未另跑 director agent）

| 代號 | 段落結構 | 次級動態載體 | 鏡頭關係 | 身分風險（髮型輪廓） |
|------|---------|------------|---------|-------------------|
| u1 | 7.2s 短促高能量：全程胸上到大腿的正面手勢舞，臉部表情變化密集（張嘴、大笑、比手勢貼臉），中段有一次明顯的上半身前後彈動 | 起始畫面的薄紗罩衫＋長髮（驅動片本人是短髮，本身幾乎沒有髮絲飄動可借） | 固定機位、正面平視，人物偶爾靠近鏡頭造成動態模糊 | **高**：驅動片本人短鮑伯頭+瀏海 vs Iris 長直髮 |
| u2 | 14.1s 中低能量：站姿為主，重心左右換、手勢比劃、偶爾轉肩，結尾有一次較大的擺動 | 敞開白襯衫下擺＋寬版褲管＋馬尾尾端 | 固定機位、正面平視、街邊實景 | **高**：驅動片本人短鮑伯頭+瀏海 vs Iris 低馬尾長髮 |
| u3 | 16.5s 最長：手勢舞為主，中段有摸頭髮/插腰的造型動作，動作幅度中等、節奏穩定 | 針織外套下襬＋長微捲髮 | **驅動片機位偏低（帶到天花板）**，起始畫面刻意用平視構圖，避開 2026-08-12 使用者明確反對的「由下往上」角度 | **低**：驅動片本人長波浪捲髮，與起始畫面長微捲髮輪廓相近 |

「停格 +5 幀」檢查：三支驅動片抽樣幀之間表情/手勢皆有明顯變化（u1 表情密度最高），沒有整段掛同一個
笑容的面具問題，三支都適合當驅動片。

### Step 5：Motion Control（2026-08-12 完成，三支一次全過）

三支都用**已核准的 v2 性感版起始畫面** + `scene_control: "image"`、`resolution: "1080p"`，
`model: kling3_0_motion_control`（回應顯示後端自動走 `mode: pro`、`background_source: input_image`）。

| 代號 | Job ID | 驅動片 media_id | 輸出 |
|------|--------|----------------|------|
| u1 | `8d38631f-e46c-4343-a8e1-1ad3ce33756d` | `a1fc2f6d-6eb7-45ee-99cf-fbe2206f406e` | 1072×1936、30fps、7.20s、無聲軌 |
| u2 | `a71f5a3c-cc3a-48bf-bed9-8bc2540b7bb7` | `ac1275bc-6159-4405-a64e-53e063895b46` | 1072×1936、30fps、14.07s、無聲軌 |
| u3 | `0512a3b4-1958-4621-a282-a3c140428a4c` | `81984981-5124-40e2-88cd-41eb1d0cceec` | 1072×1936、30fps、16.47s、無聲軌 |

**Step 4 v2 標記的「服裝改性感 → 像素審核風險升高」沒有成真**：三支都是 `completed`，沒有出現
`failed` 或 `nsfw`，一次就過。這是本 studio 第一次驗證「緞面睡裙／短版背心＋超短褲」這個尺度區間
在 `scene_control: image` 下可以通過 Motion Control 審核（先前 R6/R7 失敗的是 `scene_control: video`，
兩者不是同一個風險）。

### Step 6：混音（三支輸出皆無聲，走手動混音路徑）

`ffprobe` 確認三支輸出都**沒有音軌**（跟 Coco Wu 案例「驅動片有音軌就會自動輸出配樂」的行為不同，
本輪三支都是純視訊流），因此依 SOP Step 6 備用路徑手動混音：驅動片裁切沒有動到時間軸（只裁畫面），
所以音軌從 0s 直接對齊，`ffmpeg -map 0:v:0 -map 1:a:0 -c:v copy -c:a aac -shortest`。

### Step 7：授權與發佈限制檢查

- **驅動動作**：三支都來自第三方創作者（兩支使用者上傳、一支 IG Reel `Da74LRjxOjc`），僅供內部驗證，
  對外發佈前需評估重現程度
- **配樂**：三支都是驅動片原始配樂，**未取得商用授權**，正式發佈前必須替換
- **背景**：三支都用 `scene_control: "image"`，未借用驅動片場景，不涉及第三方場景/招牌識別性問題
  （u2 的越南街景、u1 的宴會廳都沒有被帶進成品）
- **素材存放**：驅動片原始檔與抽出的音軌只存本機工作資料夾，未進 repo

### Step 8：QA 檢核（每支抽 3 幀與已核准起始畫面圖檔並排比對）

| 檢核項 | u1 飯店 | u2 夜街 | u3 客廳 |
|--------|---------|---------|---------|
| 身分一致（臉/髮型/服裝 vs 起始畫面） | ✅ 長黑直髮、五官吻合 | ✅ 低馬尾、白襯衫+黑背心吻合 | ✅ 長黑髮、灰外套+白背心吻合 |
| **R12 身分覆蓋風險是否成真** | ❌ 未發生（驅動片本人短鮑伯頭沒有蓋過來） | ❌ 未發生 | ❌ 未發生（本來就是低風險） |
| 次級動態（髮絲/外套下襬） | ✅ 逐幀髮絲形態不同 | ✅ 襯衫下襬與髮尾有變化 | ✅ 開襟外套下襬有擺動 |
| 微表情變化 | ✅ 三幀表情皆不同（張嘴→笑→抿嘴） | ✅ 三幀不同 | ✅ 三幀不同 |
| 手部 | ✅ 無明顯崩壞 | ⚠️ 7.0s 比手勢處手指有輕微沾黏/模糊 | ⚠️ 15.5s 握拳處手指細節糊成一塊 |
| 景別 | ✅ 收在大腿上方，腿未被截斷 | ✅ 未截斷 | ✅ 未截斷 |
| 背景 | 靜止（`scene_control: image` 預期行為） | 靜止 | 靜止 |
| 規格 | H.264/AAC 44.1kHz、1072×1936、30fps | 同左 | 同左 |

**兩處手部瑕疵都出現在快速手勢的動態模糊幀**，不是全片性崩壞；若要修，做法是後製把該 0.2–0.3 秒
剪掉或做速度微調，不需要重跑整支 Motion Control。**本輪未做 R6 那種 `ffmpeg` 漂移運鏡後製**——
R6 案例使用者比對後認為差異不大，這次先給原始輸出，需要再補做。

### 產出檔案

- `kols/iris-chen/videos/dance_clone_u1_hotel/iris_dance_clone_u1_ig_reel.mp4`（1072×1936、30fps、7.20s）
- `kols/iris-chen/videos/dance_clone_u2_night_street/iris_dance_clone_u2_ig_reel.mp4`（1072×1936、30fps、14.07s）
- `kols/iris-chen/videos/dance_clone_u3_home/iris_dance_clone_u3_ig_reel.mp4`（1072×1936、30fps、16.47s）

三支皆含驅動片原始配樂音軌，未經授權，僅供內部驗證。
