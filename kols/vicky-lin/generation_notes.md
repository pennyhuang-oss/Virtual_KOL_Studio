...
| 訓練圖 | 第三輪參考圖已生成（v3_ 開頭 8 張，見下方 2026-07-25 第三輪記錄；第二輪 8 張原圖仍保留在同目錄供對照），**等待使用者確認選用哪一輪** | **PENDING** |
| 已生成圖片數量 | 16（第二輪 8 張 + 第三輪 8 張，皆為臉部參考圖，待使用者審核比較，尚未進入 Soul 訓練） | **PENDING** |
| 已生成影片數量 | 0 | **PENDING** |
...
1. ~~選定圖片生成模型並小規模測試~~ 已完成：第二輪、第三輪皆使用 `soul_2`（無 soul_id，一次性角色參考圖）
2. **等待使用者比較第二輪（`01_`–`08_`）與第三輪（`v3_01_`–`v3_08_`）參考圖，確認滿意的方向**——這是強制的人工確認關卡，不可跳過，也不可由本輪自動判定「哪一輪比較好」
