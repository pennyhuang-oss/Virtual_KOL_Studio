...
> ### ⚠️ 這句話要先跟客戶確認再寫進旁白
>
> **「App 上面可以放女生自己的短影音」這個功能，我在原片裡沒有看到任何證據。**
> 逐格看完 65.7 秒，只有 S16 的字卡提到「可觀看**相簿**」（照片），底部的「廣場」和「我的」
> 兩個分頁從頭到尾沒出現過，無從判斷裡面有沒有短影音功能。
>
> 這是客戶的官方教學片，**講一個不存在的功能是實際的問題**。動工前請 Penny 或主管跟客戶確認：
> - **有短影音功能** → 照主管的原意寫，R10 那支就是最好的示範素材
> - **只有相簿（照片）** → 旁白改成「點大頭貼可以看她的檔案跟相簿」，舞蹈影片改成
>   「她放在檔案裡的其中一支影片」這種不斷言功能的講法，或改回方向 A 的慶祝式插入
>
> 順便建議：確認的時候一起把第 2-2 節那 6 張補圖要一要。
...
1. **「App 可以放女生短影音」這個功能存不存在？**（第 3 節方向 B 的警告，這題會決定舞蹈段怎麼寫）
2. **客戶要不要補那 6 張截圖？**（廣場 ×1、我的 ×1、完成設定剩兩步 ×2、統一 demo 資料的我的邀請卡 ×2）
3. **配音走哪條路？**（第 5 節）
4. **走方向 A、方向 B，還是混搭？** 長度、成本、人工介入程度都不一樣。
