...
**待你決定的**：配音走哪條路（第 5 節有查證結果），以及要走哪個腳本方向（第 3 節）。
...
### 2-2 原片的四個瑕疵（全部用後製解，不補圖）

> **⚠️ 2026-08-24 Penny 確認：補圖確定不補。** 我原本提過可以請客戶重出約 6 張截圖，
> 現在這條取消了——**不要再去跟客戶要圖**，下面四項一律走後備作法。

| # | 瑕疵 | 怎麼解 |
|---|---|---|
| 1 | **狀態列一路跳**：開頭是紅色錄影中膠囊，之後 `2:55 → 3:34 → 3:46 → 3:37 → 3:38`，電量從 94% 掉到低電量 | **全片蓋一條統一的假狀態列**（固定時間、滿格訊號、固定電量），一次解決三個問題 |
| 2 | **「我的邀請」卡片顯示松山區，但發布表單填的是信義區**，人數也從 2 變 3（兩次錄製混剪） | 卡片的**區域欄位用局部放大裁切避開**，旁白與字卡**全程不提區域名**、只講人數變化 |
| 3 | **「完成設定」寫「再三步就能開始」但只演一步** | 該頁**標題用局部放大裁掉**，旁白不要出現「三步」這個數字，改講「照著填完，繼續」 |
| 4 | **「廣場」「我的」兩個分頁從頭到尾沒出現過** | 只在底部 tab 上**打字標名稱、不進入頁面**，旁白改成「底下五個分頁，廣場跟我的之後自己點進去看」（生成工單 VO-07 有備援句） |
...
>
> 「廣場」和「我的」兩個分頁沒畫面這件事不補圖了，走第 2-2 節的後備作法。
...
> **2026-08-24 Penny 確認：這題由你自己決定。** 下面是我查證過的所有選項與建議路徑，
> 但**都沒有實測過**——你可以照走，也可以自己找別的方法，不用回頭問我們。
...
只剩一個真正的決定：**走方向 A、方向 B，還是混搭？** 長度、成本、人工介入程度都不一樣。

另外兩件原本掛在這裡的，已經定案了：

- **補圖**：確定不補，四個瑕疵一律走第 2-2 節的後備作法
- **配音**：交給你決定，第 5 節是查證結果供參考
