...
2026-08-28（U-03）確立。ChatGPT 的覆核在**可驗證的事**上極為有效——數值、結構、
物理一致性、跨欄位矛盾，八輪下來每一條實測都屬實。但有一類判斷它抓不到，
因為那不是對錯問題，是**這個角色該長什麼樣**的問題：

| 交給 ChatGPT 覆核 | 必須由使用者拍板 |
|------------------|----------------|
| 數字是否與 JSON 一致 | **五官與臉部骨架** |
| 欄位之間有沒有矛盾 | 身材規格（罩杯、比例）取捨 |
| 場景／光線在物理上成不成立 | 這張臉像不像庫裡的其他人 |
| 規則有沒有被繞過 | 妝感、氣質、整體「感覺」 |
| 流程與 gate 的設計 | 髮色明度這種審美刻度 |

**觸發條件**：只要問題的答案是「看起來對不對」而不是「算起來對不對」，
就要用 `AskUserQuestion` 問使用者，不要自己決定，也不要送去給 ChatGPT。

**已發生的實例**：Phase A Round 2 的出圖，validator exit 0、語意覆核 20/20、
ChatGPT 連續八輪覆核全數放行——但使用者一看就指出「這張臉跟 Rainie 太像了」。
骨架收斂是 seedream 的預設行為，覆核流程完全測不到。
（`sophia-tseng-face` 的 description 顯示同一件事更早就發生過一次。）
