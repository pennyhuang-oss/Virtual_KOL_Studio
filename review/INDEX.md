...
## 🛑 三條線共用：轉傳訊息一律用程式碼區塊（2026-08-31 使用者要求）

**原話：「以後要丟給 ChatGPT 複核的請求訊息，你都要弄成我可以直接一鍵複製的形式。」**

**不分哪一條線**，要使用者轉傳給 ChatGPT 的那段訊息，一律寫在 fenced code block 裡
（三個反引號），介面才會出現複製鈕。區塊內用純文字、**不要有反引號**，
並且自帶完整路徑（repo ／ 分支 ／ 檔名），讓他不用回頭問「是哪個 repo」。
